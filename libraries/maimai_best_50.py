...
class ChartInfo(object):
    ...
    def __init__(self, id: str, title: str, level: int, achievement: float, rate: str, ra: int, fc: int, fs: int, ds: float):
        self.id = id
        self.title = title
        self.level = level
...
class DrawBest:
    ...
    def whiledraw(self, data: BestList, xi: int, yi: int) -> Image.Image:
        ...
        for n, i in enumerate(data.data):
            ...
            rate = Image.open(os.path.join(self.pic_dir, f'UI_GAM_Rank_{i.rate}.png')).convert('RGBA')
            ...
            if i.fc:
                fc = Image.open(os.path.join(self.pic_dir, f'UI_MSS_MBase_Icon_{i.fc}.png')).convert('RGBA')
                self._im.alpha_composite(fc.resize((int(fc.size[0] * 0.8), int(fc.size[1] * 0.8))), (x + 220, y + 90))
            if i.fs:
                fs = Image.open(os.path.join(self.pic_dir, f'UI_MSS_MBase_Icon_{i.fs}.png')).convert('RGBA')
    ...
    async def draw(self):
        ...
        for n, i in enumerate(['Credit to', 'XybBot & Diving-fish', 'Generated by', f'{NICKNAME if isinstance(str, NICKNAME) else list(NICKNAME)[0]} BOT']):
